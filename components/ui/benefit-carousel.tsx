...
export function BenefitGrid({ benefits }: BenefitGridProps) {
  ...
  useEffect(() => {
    const autoHoverInterval = setInterval(() => {
      // Don't trigger auto-hover if user is manually hovering
      if (hoveredIndex === null) {
        const randomIndex = Math.floor(Math.random() * benefits.length);
        setAutoHoveredIndex(randomIndex);
        
        // Clear auto-hover after 8 seconds
        setTimeout(() => {
  ...
  }, [benefits.length, hoveredIndex]);

  // Combine manual and auto hover states
  const effectiveHoveredIndex = hoveredIndex !== null ? hoveredIndex : autoHoveredIndex;

  return (
    <section ref={sectionRef} className="py-20 px-4 bg-gradient-to-br from-[#f8fdfc] to-[#e8f4f8]">
          ...
          {benefits.map((benefit, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 opacity-10 pointer-events-none">
                <Image 
                  src={index === 1 ? `/images/vegan.jpg` : `/images/${benefit.title.replace(/%/g, '%25').replace(/\s+/g, '%20')}.png`}
                  alt={benefit.title}
                  fill
